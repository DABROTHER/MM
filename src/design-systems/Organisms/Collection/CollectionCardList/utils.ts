  ...
  if (isShow) {
    if (design === 'multi') {
      return {
        ...
        className: 'w-full slg:w-[81%] grid-cols-1 md:grid-cols-2 lmd:grid-cols-4 slg:grid-cols-4 xlg:grid-cols-4',
        fileClassName: '!h-[182px] xlg:!h-[212px]  xl:!h-[262.4px]',
        cardInfoMT: 'mt-[3px] mb-1',
      }
    } else if (design === 'grid') {
      return {
        className:
      ...
      return {
        className: 'w-[60%] smd:w-[65%] md:w-[78%] ',
      }
    }
  } else {
    if (design === 'multi') {
